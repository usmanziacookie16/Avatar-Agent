...
class AvatarController {
  constructor(containerId) {
    ...
    this.morphTransitionSpeed = 0.5; // INCREASED: Faster mouth transitions (was 0.25)
  ...
  // ==================== HYBRID LIP SYNC (F1/F2 + SPECTRAL) ====================
  ...
  connectAudioSourceForLipSync(sourceNode) {
    ...
    try {
      if (!this.analyser) {
        this.analyser = sourceNode.context.createAnalyser();
        this.analyser.fftSize = 1024; // REDUCED: Faster processing (was 2048)
        this.analyser.smoothingTimeConstant = 0.1; // REDUCED: Much snappier reaction (was 0.5)
        this.frequencyData = new Uint8Array(this.analyser.frequencyBinCount);
      }

      sourceNode.connect(this.analyser);
      
      if (!this.isAnalyzingAudio) {
  ...
  continuousLipSyncAnalysis() {
    if (!this.isAnalyzingAudio || !this.isSpeaking) {
      this.isAnalyzingAudio = false;
    ...
    // High refresh rate for snappy sync
  ...
  hybridDetection(data) {
    ...
    // 1. Silence Check (Lowered threshold for better start/stop detection)
    if (avgEnergy < 5) return 'Silence';
    
    // 2. Closed Mouth Consonants (Snappier detection)
    if (avgEnergy < 25) {
        const r = Math.random();
        if (r < 0.4) return 'M';
        if (r < 0.8) return 'B';
  ...
  findPeaks(data, binSize) {
    const peaks = [];
    const threshold = 40; // Lowered to capture quieter harmonics
  ...
  startSpeaking() {
    ...
    if (!this.isAnalyzingAudio && this.lipSyncEnabled) {
      this.isAnalyzingAudio = true;
      this.continuousLipSyncAnalysis();
    }
  }
